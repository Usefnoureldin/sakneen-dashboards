...
export function InviteForm({ slug, clientName }: { slug: string; clientName: string }) {
  const [state, action, pending] = useActionState(inviteClientUserAction, initial);
      ...
      <div className="rounded-xl border border-status-approved bg-pill-approved-bg p-5 space-y-4">
        <div>
          ...
          <p className="text-sm text-slate-700 mt-1">
            Added to {clientName} as a client user.
          </p>
        </div>
        <div className="rounded-md border border-slate-200 bg-white p-4">
          ...
          <p className="text-xs text-slate-600 mt-2">
            Share this with the user out-of-band (WhatsApp, in person). It will not be shown again.
          </p>
        </div>
  ...
  return (
    <form action={action} className="flex flex-col gap-4">
      ...
      <div className="flex justify-between items-center">
        <Link
          href={`/admin/clients/${slug}`}
          className="text-sm text-slate-600 hover:text-charcoal"
        >
          Cancel
        </Link>
        <button
